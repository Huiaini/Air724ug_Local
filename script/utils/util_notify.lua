...
--- 构建通知头
-- @return (string) 通知头; 关闭或失败返回 ""
local function buildNotifyHeader()
    local function getTail(raw, tail_len)
        if type(raw) ~= "string" or raw == "" then
            return nil
        end
        raw = tostring(raw):gsub("%D", "")
        raw = raw:gsub("^86+", "")
        if #raw < tail_len then
            return nil
        end
        return string.sub(raw, -tail_len)
    end

    local tpl = config.NOTIFY_HEADER
    ...
    -- 未使用 {tail} 变量时, 视为固定通知头
    if not string.find(tpl, "{tail}", 1, true) then
        return tpl
    end

    local tail_len = tonumber(config.NOTIFY_HEADER_TAIL_LEN) or 4
    if tail_len < 1 then tail_len = 4 end

    -- 优先使用本机号码, 再尝试 IMSI, 再尝试 ICCID
    local tail = getTail(sim.getNumber(), tail_len)
        or getTail(config.NUMBER, tail_len)
        or getTail(sim.getImsi(), tail_len)
        or getTail(sim.getIccid(), tail_len)
    if not tail then
        local fallback = config.NOTIFY_HEADER_FALLBACK
    ...
    return string.gsub(tpl, "{tail}", tail)
end

--- 构建设备信息字符串, 用于追加到通知消息中
-- @param opts (table) 可选项
--   opts.hide_signal: true 时不追加信号和频段
--   opts.hide_boot_time: true 时不追加开机时长
--   opts.hide_temperature: true 时不追加温度
--   opts.hide_voltage: true 时不追加电压
-- @return (string) 设备信息
local function buildDeviceInfo(opts)
    opts = opts or {}
    local msg = "\n"
    ...
    if not opts.hide_boot_time then
        -- 开机时长
    ...
    if not opts.hide_temperature then
        -- 温度
...
-- @param opts (table) 发送选项
--   opts.keep_full_append_info: true 时保留完整附加信息
-- @return (boolean) 是否发送成功/不需要重发
function send(msg, channel, opts)
    opts = opts or {}
    ...
    -- 通知头追加手机号后四位
    msg = buildNotifyHeader() .. msg

    -- 通知内容追加设备信息
    if config.NOTIFY_APPEND_MORE_INFO then
        -- 默认统一精简附加信息（短信/来电/控制/存活等）
        -- 开机通知通过 opts.keep_full_append_info 保留完整附加信息
        local append_opts = nil
        if not opts.keep_full_append_info then
            append_opts = {
                hide_signal = true,
                hide_boot_time = true,
        ...
        msg = msg .. buildDeviceInfo(append_opts)
    end
...
-- @param msg 消息内容
-- @param channels 通知渠道
-- @param opts 发送选项, 透传到 send
function add(msg, channels, opts)
    ...
    for _, channel in ipairs(channels) do
        table.insert(msg_queue, { channel = channel, msg = msg, retry = 0, opts = opts })
    end
    sys.publish("NEW_MSG")
    ...
    while true do
        ...
        if next(msg_queue) ~= nil and net.getState() == "REGISTERED" then
            ...
            if item.retry > (config.NOTIFY_RETRY_MAX or 100) then
                ...
                result = send(item.msg, item.channel, item.opts)
